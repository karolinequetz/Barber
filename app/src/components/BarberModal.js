...
const DateInfo = styled.View`flex-direction: row;`;
...
const TimeItemText = styled.Text`font-size: 16px;`;
...
export default ({ show, setShow, user, service }) => {
	...
	useEffect(
		() => {
			if (user.available) {
				let daysInMonth = new Date(selectedYear, selectedMonth + 1, 0).getDate(); //pegar o último dia do mês
				let newListDays = [];
				...
				<ModalBody>
					...
					<ModalItem>
						<DateInfo>
							...
							<DateNextArea onPress={handleHightDateClick}>
									...
									>
										<TimeItemText
											style={{
												...
												weight: item === selectedHour ? 'bold' : 'normal'
											}}
										>
											{item}
										</TimeItemText>
